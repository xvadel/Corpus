...
class GapAnalyzer:
    def __init__(
        ...
        skill_model=None  # Optional[UserSkillModel] — avoids circular imports
    ):
    ...
    def analyze_gaps(
        ...
        known_concept_ids: Optional[List[str]] = None,
        ...
        Priority for known concepts:
          1. known_concept_ids (explicit override)
          2. skill_model.get_known_concept_ids(user_id) if skill_model and user_id are set
          3. Empty list (no prior knowledge assumed)
        """
        required_path = self.path_builder.build_path_for_concept(target_concept_id)
        required_ids  = [c["id"] for c in required_path]

        # Resolve known concepts
        if known_concept_ids is not None:
            known_set = set(known_concept_ids)
        elif self.skill_model is not None and user_id is not None:
            known_set = set(self.skill_model.get_known_concept_ids(user_id))
        else:
            known_set = set()
